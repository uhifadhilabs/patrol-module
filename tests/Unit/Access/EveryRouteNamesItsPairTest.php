...
namespace Uhifadhi\Patrol\Tests\Unit\Access;
 ...
 *     guarding it: the registry's listener only closes routes in an area that
 *     has PARKED the module, which is not a permission.
 *   - A ROUTE THAT NAMES A PAIR NOBODY DECLARED can never be granted, because
 *     the voter refuses a pair its catalogue does not know. The page is then
 *     shut to everybody but an administrator, and no error says so.
...
final class EveryRouteNamesItsPairTest extends TestCase
{
    ...
    public function testEveryPairARouteNamesIsOneThisModuleOrTheCoreDeclares(): void
    {
        ...
        foreach (self::routes() as [$file, $name, $pairs]) {
            foreach ($pairs as $pair) {
                ...
                // the core declares; what it must never do is name a verb of
                // one of OUR concerns that we never declared.
                if (self::declares($grant->concern) && !\in_array($pair, self::declaredPairs(), true)) {
     ...
     * route; it is skipped by the same test the core's uses — a chunk with no
     * quoted path in it.
     *
    ...
    private static function routes(): array
    {
        ...
        foreach (self::shippedSource() as $path) {
            ...
            foreach (\array_slice(explode('#[Route(', $source), 1) as $chunk) {
                ...
                if (1 !== preg_match("/name: '([a-z0-9_]+)'/", $head, $name)) {
                    continue;
                }

                preg_match_all("/#\[IsGranted\(\s*'([^']+)'/", $head, $gates);

                $routes[] = [basename($path), $name[1], $gates[1]];
            }
        }
    ...
    {
        $pairs = [];
        foreach (new PatrolConcerns()->concerns() as $concern) {
    ...
    private static function declares(string $concern): bool
    {
        foreach (new PatrolConcerns()->concerns() as $declared) {
            if ($concern === $declared->key()) {
                return true;
            }
        }
